...
def select_04():
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("SELECT e.nombre || ' ' || e.apellido AS estudiante, c.nombre AS curso, i.nota FROM inscripciones i JOIN estudiantes e ON i.estudiante_id = e.id JOIN cursos c ON i.curso_id = c.id")  
    for r in cursor.fetchall():
        print(dict(r))
    conn.close()
